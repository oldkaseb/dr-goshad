...
from utils.db import get_users, add_admin, get_admins, get_users, is_blocked
...
async def handle_broadcast_content(message: types.Message):
    ...
    from utils.db import get_users, is_blocked
    users = get_users()
    count = 0
    for user in users:
...
async def admin_handler(message: types.Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        await message.reply("❗️ لطفاً آیدی عددی کاربر را به‌صورت زیر وارد کن:\n`/admin 1234567890`", parse_mode="Markdown")
        return
